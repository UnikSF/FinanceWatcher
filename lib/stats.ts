...
/** Imprévu transactions for the month — excluded from income/expenses; shown
 *  as a detail-on-hover total. */
export function unplanned(month: string): Unplanned {
  const db = getDb();
  const items = db
    .prepare(
      `SELECT t.date, t.merchant, t.description, t.amount
       FROM transactions t JOIN categories c ON c.id = t.category_id
       WHERE c.top_category = 'Imprévu' AND strftime('%Y-%m', t.date) = ?
       ORDER BY t.date DESC`
    )
    .all(month) as Unplanned["items"];
  return { total: items.reduce((s, i) => s + Math.abs(i.amount), 0), items };
}

export type GroupSpend = { top_category: string; total: number };

/** Expense spending grouped by top category (Imprévu excluded). */
export function spendingByTopCategory(month: string): GroupSpend[] {
  const db = getDb();
  return db
    .prepare(
      `SELECT COALESCE(c.top_category, 'Autre') AS top_category, SUM(-t.amount) AS total
       FROM transactions t JOIN categories c ON c.id = t.category_id
       WHERE t.amount < 0 AND c.kind = 'expense' AND strftime('%Y-%m', t.date) = ?
         AND COALESCE(c.top_category, '') != 'Imprévu'
       GROUP BY top_category
       ORDER BY total DESC`
    )
    .all(month) as GroupSpend[];
}
