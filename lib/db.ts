...
/** 2-level grouping: a `top_category` over the leaf categories. Idempotent so
 *  it also upgrades existing DBs and never overwrites a user's manual grouping.
 *  `top_category = 'Imprévu'` is the exclusion signal (dropped from income,
 *  expenses and budgets — see lib/stats.ts). */
function topCategories(db: Database.Database) {
  const cols = db.prepare("PRAGMA table_info(categories)").all() as Array<{ name: string }>;
  if (!cols.some((c) => c.name === "top_category")) {
    db.exec("ALTER TABLE categories ADD COLUMN top_category TEXT");
  }
  // Imprévu leaf (kind=expense to satisfy the kind CHECK; top_category excludes it).
  db.prepare(
    "INSERT OR IGNORE INTO categories (name, icon, color, kind) VALUES ('Imprévu','❓','#a3a3a3','expense')"
  ).run();
  const GROUPS: Record<string, string[]> = {
    Loisir: ["Restaurants & Bars", "Entertainment", "Shopping", "Travel", "Subscriptions & Telecom"],
    Vie: ["Groceries", "Housing & Rent", "Utilities", "Health", "Education",
          "Fees & Charges", "Savings & Investments", "Income", "Transfers", "Other"],
    "Déplacement": ["Transport", "Cash Withdrawal"],
    "Imprévu": ["Imprévu"],
  };
  const set = db.prepare(
    "UPDATE categories SET top_category = ? WHERE name = ? AND (top_category IS NULL OR top_category = '')"
  );
  db.transaction(() => {
    for (const [top, names] of Object.entries(GROUPS)) for (const n of names) set.run(top, n);
  })();
}
